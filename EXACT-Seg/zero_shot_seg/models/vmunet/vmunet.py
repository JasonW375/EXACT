...
class VMUNet(nn.Module):
    def __init__(self, 
                 ...
                 num_abnormal_classes=16,  # 16个区域的异常检测（每个区域二分类）
                 depths=4, 
                 n_base_filters=32,
        ...
        # 多个分辨率的异常检测输出头  
        # self.final_conv_abn_scale1 = nn.Conv3d(n_base_filters * 4, num_abnormal_classes, kernel_size=1)  
        self.final_conv_abn_scale2 = nn.Conv3d(n_base_filters * 2, num_abnormal_classes, kernel_size=1)  
    ...
    def forward(self, x):
        ...
        for idx in range(0, len(self.decoder_levels), 2):
            ...
            # 在每个分辨率级别生成输出  
            if idx == 2:  # H/2 分辨率     
                abn_output_scale2 = self.activation_abnormal(self.final_conv_abn_scale2(x))  
                # print(f"abn_output_scale2 shape: {abn_output_scale2.shape}")  
                # multi_scale_abn_outputs.append(abn_output_scale2)  


        # 输出
        final_seg_output = self.activation_segmentation(self.final_conv_segmentation(x))  
        # print(f"final_seg_output shape: {final_seg_output.shape}") 
        final_abn_output = self.activation_abnormal(self.final_conv_abnormal(x)) 
        # print(f"final_abn_output shape: {final_abn_output.shape}") 
        # multi_scale_abn_outputs.append(final_abn_output)

        # return final_seg_output, multi_scale_abn_outputs#返回顺序：[H/2, H] 分辨率的结果 
        return (  
                final_seg_output,  
                [abn_output_scale2, final_abn_output]  # 返回低分辨率和高分辨率的异常检测结果  
            )   
