...
# -*- coding: utf-8 -*-
"""
Master evaluation script for RadChest dataset - Classification Only Version with 95% CI
仅计算分类指标，排除占位类别，包含95%置信区间
"""
...
CLS_SCRIPT   = CODE_DIR / "calc_scores_withci.py"  # 修改：使用带CI的脚本
...
def evaluate_single_file(input_file: Path, file_index: int, total_files: int):
    ...
    try:
        ...
        # 3. multi-label classification scores with 95% CI (使用处理后的CSV)
        ...
        combined = {
            ...
            "metadata": {
                ...
                "note": "仅分类指标，排除占位类别后的结果，包含95%置信区间"
            }
        }
        ...
        if cls_metrics:
            ...
            if 'macro' in cls_metrics:
                ...
                for key in ['precision', 'recall', 'f1', 'accuracy']:
                    if key in macro:
                        value_str = macro[key]  # ✅ 直接是字符串 "0.0923 [0.0800, 0.1049]"
                        print(f"      {key:12s}: {value_str}")

            # 显示per_pathology指标的前5个（如果存在）
            if 'per_pathology' in cls_metrics:
                ...
                for item in per_path:
                    ...
                    for key in ['precision', 'recall', 'f1', 'accuracy']:
                        if key in item:
                            value_str = item[key]  # ✅ 直接是字符串
                            print(f"         {key:12s}: {value_str}")
                
                if len(cls_metrics['per_pathology']) > 5:
                    print(f"\n      ... 以及其他 {len(cls_metrics['per_pathology']) - 5} 个类别")
...
def generate_summary_report(successful_files: list[Path], base_dir: Path):
        ...
        if metrics_file.exists():
            try:
                ...
                if "classification" in metrics and "macro" in metrics["classification"]:
                    ...
                    for key in ['precision', 'recall', 'f1', 'accuracy']:
                        if key in macro:
                            value_str = macro[key]  # ✅ "0.0923 [0.0800, 0.1049]"
                            summary_item[f"cls_{key}"] = value_str  # ✅ 直接保存字符串
    ...
    if summary_data:
        ...
        # 显示表格（含置信区间）
        print(f"\n📈 分类性能对比表格 (含95% CI，排除占位类别后):")
        print("-" * 120)
        print(f"{'文件名':<35} {'精确率':<30} {'召回率':<30} {'F1分数':<30}")
        print("-" * 120)
        ...
        for item in summary_data:
            ...
            def format_metric(key):
                if f'cls_{key}' in item:
                    return item[f'cls_{key}']  # ✅ 直接返回字符串
                return "N/A"
...
def main():
    """主函数，处理所有输入文件"""
    print("🚀 RadChest评估脚本 - 仅分类指标版本（含95% CI，排除占位类别）")
    ...
    if failed > 0:
        ...
        print("5. 确认calc_scores_withci.py和confidence_interval.py已正确放置")
