...
# -*- coding: utf-8 -*-
"""
Evaluation script for the MianYang cohort.

Same four stages as evaluation.py (classification, CRG, NLG), with one
difference: MianYang does not annotate *Mosaic attenuation pattern*, so that
column is dropped from both the predictions and the ground truth before the
classification and CRG stages, leaving 17 classes. NLG metrics are unaffected.
...
Example:
    ...
    python evaluation_mianyang.py \
        --prediction_jsons out_mianyang.json \
        --base_dir mianyang_workdir
"""
...
# The MianYang variant reads its label columns from the CSV header, so it works
# after the placeholder column has been dropped.
CRG_SCRIPT = CODE_DIR / "crg_score_mianyang.py"
NLG_SCRIPT = CODE_DIR / "nlg_metrics.py"

# Placeholder column, excluded from both sides before scoring.
EXCLUDED_CLASSES = [
...
def evaluate_single_file(input_file: Path, file_index: int, total_files: int):
    ...
    # Intermediates: the full 18-class tables, and the 17-class ones actually scored.
    ...
    try:
        ...
        # 2. Drop the placeholder column from both sides.
        ...
        # 3. Multi-label classification scores over the remaining 17 classes.
        ...
        # 4. CRG score, on the same filtered tables.
        print("\n4. Computing CRG score...")
        run(CRG_SCRIPT,
            "--pred_csv", csv_pred,
            "--gt_csv", gt_csv_filtered,
            "--out_json", crg_json)
        ...
        # 5. NLG metrics, scored on the unfiltered reports.
        ...
        # 6. Merge everything into one metrics.json.
        ...
        print(f"\nSummary (checkpoint-{checkpoint_num}, placeholder classes excluded):")
        print("=" * 50)
...
def generate_summary_report(successful_files: list, base_dir: Path):
    ...
    print(f"Classification and CRG exclude: {', '.join(EXCLUDED_CLASSES)}")


def main():
    print("EXACT-CHAT evaluation (MianYang)")
    ...
    if successful_files:
        ...
        print("   inferred.csv               predicted labels, placeholder dropped")
        print("   ground_truth_filtered.csv  reference labels, placeholder dropped")
        print("   classification_scores.json per-class and overall metrics")
        print("   crg_scores.json            CRG score")
        print("   nlg_scores.json            BLEU / ROUGE-L / METEOR / CIDEr")
