...
def evaluate_single_file(input_file: Path, file_index: int, total_files: int):
    ...
    # 定义文件路径
    # 输入文件（准备脚本生成的）
    pred_json_transformed = prep_dir / "result_transformat.json"  # infer.py和NLG都使用这个
    gt_json = prep_dir / "ground_truth.json"                      # NLG评估使用
    gt_csv = prep_dir / "ground_truth.csv"                        # 分类评估使用
    
    # 输出文件（评估结果）
    csv_pred = prep_dir / "inferred.csv"                          # infer.py输出
    cls_json = prep_dir / "classification_scores.json"            # 分类评估结果
    crg_json = prep_dir / "crg_scores.json"                       # CRG评估结果
    nlg_json = prep_dir / "nlg_scores.json"                       # NLG评估结果
    final_json = prep_dir / "metrics.json"                        # 最终合并结果
    ...
    try:
        ...
        # 4. NLG metrics (使用转换后的文件 - 这是关键修复！)
        print("\n4. Computing NLG metrics...")
        print("   🔧 修复：使用转换后的文件进行NLG评估")
        run(NLG_SCRIPT,
            "--pred_json", pred_json_transformed,  # 修复：使用转换后的文件而不是原始文件
            "--gt_json", gt_json,
            "--out_json", nlg_json)
        ...
        # 打印简要结果
        print(f"\n📊 评估结果总览 (Checkpoint-{checkpoint_num}):")
        print("=" * 50)
...
def main():
    ...
    if failed > 0:
        print("\n🔧 问题诊断建议:")
        print("1. 分类警告：如果看到精确率警告，检查分类器阈值设置")
        print("2. 数据问题：确保ground truth数据正确准备")
        print("3. 模型问题：检查分类器模型是否正确加载")
        print("4. 文件夹模式：确保文件模式匹配正确的文件")
